set -x
# ---------------------------------------------------------------------------
# RRG reverse-reasoning RL — native ANSWER-RECOVERY tasks (plain reasoning), 8 GPUs.
# Two-channel GiGPO:
#   STEP  = action-recovery margin (env, 8B reader SEES the screenshot)
#   TRAJ  = generative answer-recovery RECALL (reward_manager=rrg; blind 8B reader assembles
#           the gold-schema answer from the policy's reasonings -> field-level recall in [0,1]).
# This is the E1/E4-validated non-saturated trajectory reward (the completion judge is dead).
# gamma=0 -> step_rewards = immediate per-step margin. Policy init = patched AMEX plain-SFT-4B.
# Train pool = data/rrg-train (397 answer-bearing tasks); val = data/rrg-val (40 tasks).
#
...
val_data_size=${VAL_DATA_SIZE:-8}        # val tasks per batch (G=1 for val); 40 val tasks total
group_size=${GROUP_SIZE:-8}              # G rollouts per task (env.rollout.n) -> GiGPO group size
mode="mean_norm"

# steps_per_epoch = train_rows / train_data_size. RRG train pool = 397 tasks -> ~25 steps/epoch
# covers each distinct task ~once per epoch (env re-samples real episodes every batch via
# num_episodes=null; the parquet content is dummy, only its ROW COUNT sets the step budget).
steps_per_epoch=${STEPS_PER_EPOCH:-25}
train_rows=$(( steps_per_epoch * train_data_size ))
val_rows=$val_data_size
total_epochs=${TOTAL_EPOCHS:-3}
num_episodes=${RRG_NUM_EPISODES:-null}   # null = all 397; set small for a smoke test

# --- full-answer-correctness levers (see memory rrg-completeness-levers) ---
# #1 train-reward reader token budget (yaml default already 2048; override here if needed).
ANSWER_MAX_TOKENS=${ANSWER_MAX_TOKENS:-2048}
...
# #3 GiGPO trajectory-reward shaping for completeness. Offline-best = threshold (rrg-completeness-levers).
...
# per-step answer-field credit (GiGPO micro channel); off by default = plain-recall baseline.
ANSWER_STEP_CREDIT=${ANSWER_STEP_CREDIT:-false}    # true to enable
...
python3 -m verl.trainer.main_ppo \
    ...
    env.rrg.train_task_root=$RRG_DATA_ROOT/data/rrg-train \
    env.rrg.val_task_root=$RRG_DATA_ROOT/data/rrg-val \
    ...
    trainer.experiment_name=${EXPERIMENT_NAME:-gigpo-answer-4b-g8-stepcredit} \
